...
const ProjectCard = ({ data }: { data: Project }): JSX.Element => {
  ...
  return (
    <Link href={data.url || ''} target={data.url?.startsWith('http') ? '_blank' : '_self'}>
      <li className="relative rounded-lg bg-bianca-200 p-3 text-bianca-900 shadow-md drop-shadow-md transition-all hover:-top-2 hover:z-20 hover:shadow-lg hover:drop-shadow-lg">
        <div className="flex">
          <div className="flex w-full grow flex-col break-words">
            <h3 className="text-xl font-semibold">{data.name}</h3>
            <p className="text-bianca-700">{data.description}</p>
          </div>
